...
Bridge.prototype = {
  ...
  registerProxy: function(client, provider, callback) {
    if (!provider.domain) {
      return send(
        client,
        [
  ...
  requestDatabases: function(client, callback) {
    ...
    callback(null, this._aggregateDatabases());
  },
  ...
  requestObjectStores: function(client, db, callback) {
    ...
    provider.client.emit('objectStores', owner.database, function(objectStores) {
      callback(null, objectStores);
    });
  },

  requestAll: function(client, db, store, options, callback) {
    if (!callback) {
      return client.emit('error', '"all" event requires callback');
    }
    // create request
